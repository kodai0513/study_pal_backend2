...
@router.post(
    "/", response_model=CreateArticleResp, status_code=status.HTTP_201_CREATED
)
def create_article(
    req: CreateArticleReq, session: SessionDep, auth: AuthDep
) -> CreateArticleResp:
    try:
        return CreateArticleResp.model_validate(
...
@router.patch(
    "/{article_id}",
    response_model=UpdateArticleResp,
...
def update_article(
    ...
    try:
        return UpdateArticleResp.model_validate(
